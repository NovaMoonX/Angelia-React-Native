...
import {
  generatePublicDisplayName,
  isValidPublicDisplayName,
  normalizePublicDisplayNameInput,
...
function normalizeUserPublic(pub: UserPublic): UserPublic {
  const publicDisplayName =
    pub.publicDisplayName?.trim() ||
    generatePublicDisplayName(pub.avatar ?? 'moon');
  return { ...pub, publicDisplayName };
}

function normalizeUserPrivate(priv: UserPrivate | null): Pick<UserPrivate, 'email' | 'funFact' | 'status' | 'hideNameFromNonConnections' | 'hideAvatarFromNonConnections'> {
  return {
    email: priv?.email ?? '',
    funFact: priv?.funFact ?? '',
    status: priv?.status ?? null,
    hideNameFromNonConnections: priv?.hideNameFromNonConnections ?? DEFAULT_IDENTITY_PRIVACY.hideNameFromNonConnections,
    hideAvatarFromNonConnections: priv?.hideAvatarFromNonConnections ?? DEFAULT_IDENTITY_PRIVACY.hideAvatarFromNonConnections,
  };
}
...
function mergeUserDocs(
  ...
  const normalizedPub = normalizeUserPublic(pub);
  return {
    ...normalizedPub,
    ...normalizeUserPrivate(priv),
...
export async function createUserProfile(userData: NewUser): Promise<void> {
  ...
  const privateData: UserPrivate = {
    ...
    status: null,
    hideNameFromNonConnections: userData.hideNameFromNonConnections ?? DEFAULT_IDENTITY_PRIVACY.hideNameFromNonConnections,
    hideAvatarFromNonConnections: userData.hideAvatarFromNonConnections ?? DEFAULT_IDENTITY_PRIVACY.hideAvatarFromNonConnections,
  };
...
export async function updatePublicDisplayName(uid: string, name: string): Promise<void> {
  const normalized = normalizePublicDisplayNameInput(name);
  if (!isValidPublicDisplayName(normalized)) {
    throw new Error('Public display name must be between 3 and 40 characters.');
  }
  await updateDoc(doc(getDb(), 'usersPublic', uid), { publicDisplayName: normalized });
}

export async function updateIdentityPrivacy(
  uid: string,
  data: Partial<Pick<UserPrivate, 'hideNameFromNonConnections' | 'hideAvatarFromNonConnections'>>,
): Promise<void> {
  await updateDoc(doc(getDb(), 'usersPrivate', uid), data);
}

/** Returns another user's uid if they already own this public display name. */
export async function findPublicDisplayNameOwner(
  name: string,
  excludeUid: string,
): Promise<string | null> {
  const normalized = normalizePublicDisplayNameInput(name);
  const snap = await getDocs(
    query(
      collection(getDb(), 'usersPublic'),
      where('publicDisplayName', '==', normalized),
      limit(2),
    ),
  );
  for (const docSnap of getSnapshotDocs(snap)) {
    if (docSnap.id !== excludeUid) return docSnap.id;
  }
  return null;
}
