...
export interface AuthorPostActivityValue {
  ...
  refreshSeenState: () => Promise<void>;
  markPostsSeen: (postIdsToMark: string[]) => Promise<void>;
  markConversationSeen: (postIdsToMark?: string[]) => Promise<void>;
}
...
export function AuthorPostActivityProvider({ children }: { children: React.ReactNode }) {
  ...
  const [postActivitySeenAt, setPostActivitySeenAt] = useState<number | null>(() => {
    return seenStateCache[currentUserId ?? '']?.postActivitySeenAt ?? null;
  });
  const [seenMaps, setSeenMaps] = useState<SeenMaps>(() => {
    return seenStateCache[currentUserId ?? '']?.seenMaps ?? { reactionsByPostId: {}, privateNotesByPostId: {}, conversationByPostId: {} };
  });
  ...
  const refreshSeenState = useCallback(async () => {
    ...
    const runRefresh = async () => {
      ...
      if (parsedPostActivitySeen === 0 && rawPostActivitySeen) {
        ...
        seenStateCache[currentUserId] = {
          postActivitySeenAt: migratedTimestamp,
      ...
      if (subscribedPostIds.length === 0) {
        ...
        seenStateCache[currentUserId] = {
          postActivitySeenAt: seenStateCache[currentUserId]?.postActivitySeenAt ?? null,
          seenMaps: emptySeen,
        };
        return;
      }
    ...
    const inflightPromise = runRefresh().finally(() => {
  ...
  }, [hasUnread, markConversationSeen, markPostsSeen, refreshSeenState, summaries, unreadDetailsByPostId, unreadPostIds]);
